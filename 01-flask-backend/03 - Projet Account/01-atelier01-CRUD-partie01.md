...
Ce guide vous expliquera comment déployer une application Flask avec une base de données PostgreSQL sur une instance Amazon EC2. Nous allons détailler chaque étape, depuis la création de l'instance EC2 jusqu'à la configuration des points de terminaison et le lancement de l'application.
...
#### 2.4 **Installer Nginx (pour servir l'application)**
```bash
sudo apt install nginx -y
```

#### 2.5 **Configurer PostgreSQL**
...
#### 3.1 **Créer la structure du projet sur l'instance**
```bash
...
#### 3.3 **Installer Flask et les autres dépendances**
```bash
pip install Flask SQLAlchemy psycopg2-binary Flask-Migrate gunicorn
```

#### 3.4 **Créer la structure du projet Flask**
```bash
...
#### 3.5 **Ajouter la configuration de la base de données dans `config.py`**
```python
import os
...
#### 3.6 **Définir les modèles dans `models.py`**
```python
...
#### 3.7 **Initialiser l'application dans `__init__.py`**
```python
...
#### 3.8 **Définir les routes dans `routes.py`**
```python
...
#### 3.9 **Créer un fichier Gunicorn pour exécuter l'application**
...
#### 3.10 **Testez votre application en local**
...
#### 5.1 **Créer un fichier de service pour Gunicorn**
...
#### 5.2 **Démarrer et activer le service Gunicorn**
...
#### 5.3 **Vérifier le statut du service**
...
#### 6.1 **Accéder à l'application depuis le navigateur**
...
#### 6.2 **Tester les endpoints définis dans `routes.py`**
...
#### 6.3 **Sécuriser les communications avec HTTPS (optionnel)**
