...
Dans cette section, vous allez tester l'application Spring Boot en utilisant des requêtes HTTP pour gérer les comptes associés aux clients. Vous allez utiliser Postman ou un autre outil similaire pour envoyer les requêtes POST, GET, PUT, et DELETE. Voici les étapes et les exemples de JSON à utiliser pour chaque opération.

---

### 1️⃣ Insertion des Comptes via HTTP (POST) ➕

Après avoir inséré les clients dans la base de données, vous pouvez commencer à créer les comptes associés via des requêtes POST.
...
### 2️⃣ Continuer à tester les différentes fonctionnalités en fonction des opérations précédentes.
