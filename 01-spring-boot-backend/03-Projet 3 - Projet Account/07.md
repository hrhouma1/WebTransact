J'ai ajouté les annotations nécessaires pour définir la relation `OneToMany` et `ManyToOne` entre les classes `Customer` et `Accounts`. Voici le code mis à jour :
